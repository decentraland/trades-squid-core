...
psql -v ON_ERROR_STOP=1 --username "$DB_USER" --dbname "$DB_NAME" --host "$DB_HOST" --port "$DB_PORT" <<-EOSQL
  ...
  GRANT USAGE ON SCHEMA $NEW_SCHEMA_NAME TO $MARKETPLACE_SERVER_API_READER_USER, $SQUID_READER_USER, $MARKETPLACE_TRADES_MV_ROLE, $ATLAS_SERVER_READONLY_USER;
  ...
  ALTER DEFAULT PRIVILEGES FOR ROLE $NEW_DB_USER IN SCHEMA $NEW_SCHEMA_NAME
    GRANT SELECT, TRIGGER ON TABLES TO $MARKETPLACE_SERVER_API_READER_USER, $SQUID_READER_USER, $MARKETPLACE_TRADES_MV_ROLE, $ATLAS_SERVER_READONLY_USER;
